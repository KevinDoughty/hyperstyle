...
import { parseNumber, mergeNumbers } from "./number-handler.js";
import { parseAngle, parseLengthOrPercent, parseLength, mergeDimensions } from "./dimension-handler.js";
import { makeMatrixDecomposition } from "./matrix-decomposition.js";
import { quat, composeMatrix } from "./matrix-interpolation.js";

import { addPropertiesHandler } from "./property-interpolation.js";
...
function mergeMatrices(left, right) {
	...
	return [
		...
		rightArgs,
		function(list) {
			var quaternion = quat(leftArgs[0][3], rightArgs[0][3], list[5]);
			var mat = composeMatrix(list[0], list[1], list[2], quaternion, list[4]);
...
export function mergeTransforms(left, right) { // ugly values, of course
	var matrixModulesLoaded = makeMatrixDecomposition && true;
	console.log("M E R G E   left:%s; right:%s;",JSON.stringify(left),JSON.stringify(right));
	...
	console.log("T Y P E S:%s;",JSON.stringify(types));
	return [leftResult, rightResult, function(list) {
		return list.map(function(args, i) {
			console.log("mergeTransforms args:%s;",JSON.stringify(args));
			var stringifiedArgs = args.map(function(arg, j) {
				return types[i][1][j](arg);
			}).join(',');
			console.log("stringified:%s;",JSON.stringify(stringifiedArgs));
			console.log("types:%s;",JSON.stringify(types));
